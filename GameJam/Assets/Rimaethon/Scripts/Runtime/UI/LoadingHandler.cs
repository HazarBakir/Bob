...
namespace Rimaethon.Runtime.UI
{
    ...
    public class LoadingHandler : PersistentSingleton<LoadingHandler>
    {
        ...
        private readonly WaitForSeconds _waitForHalfSecond= new WaitForSeconds(0.5f);
        ...
        private IEnumerator LoadScene(int sceneIndex)
        {
            ...
            while (_sceneUnload.progress < 0.9f)
            {
                yield return null;
            }
        ...
        private IEnumerator MoveLoadingScreen( Vector2 targetPivot, float duration)
        {
            ...
            Vector2 startPivot = loadingScreenImage.rectTransform.pivot;
            Vector2 halfWayPivot =(targetPivot+startPivot)/2 ;
            ...
            if (_isRespawnScreen)
            {
                EventManager.Instance.Broadcast(GameEvents.OnPlayerRespawned);
            }

            while (!_isScreenShouldContinue)
            {
                yield return null;
            }
